...
class MissingReportViewer extends Component {
  constructor(props) {
    super(props);
    this.state={
       allData:[],
  ...
  componentDidMount(){
    ...
    const data ={userId}
    this.props.actions.getAllReportData(`${config.port}/api/sendallmissingreports`,data)
    .then((res)=>{
      {
        this.setState({
        isLoading:false,
  ...
  renderComponent=()=>{
    ...
    return (
      ...
      <h1 style={{color:'#ffff',textAlign:'center'}} >Missing reports </h1>
    ...
    return (
      ...
      <h1 style={{color:'#ffff',textAlign:'center'}}  >Missing reports are</h1>
        <Container style={divStyle} >
...
export default connect(mapStateToProps,mapDispatchToProps)(MissingReportViewer);
