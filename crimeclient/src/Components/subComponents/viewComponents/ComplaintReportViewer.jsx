...
class ComplaintReportViewer extends Component {
  constructor(props) {
    super(props);
    this.state={
       allData:[],
  ...
  componentDidMount(){
    ...
    const data ={userId}
    this.props.actions.getAllReportData(`${config.port}/api/sendallcomplaintreports`,data)
    .then((res)=>{
      {this.setState({
        isLoading:false,
      })}
  ...
  renderComponent=()=>{
    ...
    return (
      ...
      <h1 style={{color:'#ffff',textAlign:'center'}} >Complaint reports</h1>
    ...
    {allData.map((item,index)=>(
      ...
      <ListGroup flush>
      <ListGroupItemHeading style={{textAlign:'center'}} >Complaint<Badge style={{fontSize:20}} color="dark">Report #  {index+1}</Badge></ListGroupItemHeading>
     ...
     </ListGroup><Button color='dark' readOnly>{item.adminStatus}</Button>
      </Card>
    </Col> 
    ))}
...
export default connect(mapStateToProps,mapDispatchToProps)(ComplaintReportViewer);
